            ...
            <div class="animate-fade-up">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 30px;">
                    ...
                    <div class="card glass-panel" style="padding: 20px; text-align: center;">
                        ...
                        <span style="font-size: 2.5rem; font-weight: 800; color: var(--text-main);">2</span>
                    </div>
                </div>
...
<main style="padding-top: 100px; padding-bottom: 60px;">
            ...
            <div class="animate-fade-up">
                ...
                <div class="card">
                    ...
                    <ul style="list-style: none;">
                        ...
                        <li style="padding: 15px 0; display: flex; justify-content: space-between;">
                            <span>Account created securely</span>
                ...
                <div class="card" style="margin-top: 30px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color); padding-bottom: 10px; margin-bottom: 16px;">
                        ...
                        <button id="syncAwsBtn" class="btn btn-primary" style="padding: 6px 12px; font-size: 0.85rem;">Sync to DynamoDB</button>
                    </div>
    ...
    init: () => {
        ...
        if (email) {
            document.getElementById('dashUserName').innerText = email;
        }
        ...
        if (syncBtn) {
            syncBtn.addEventListener('click', async () => {
                ...
                try {
                    // Get the JWT token we captured from Cognito
                    const idToken = localStorage.getItem('ic_session_token');
                    
                    if (!idToken) throw new Error("No session token found. Please log in again.");
                    ...
                    const response = await fetch("https://nwmdojx1m4.execute-api.ap-south-1.amazonaws.com/prod/users", {
                        ...
                        headers: {
                            ...
                            'Authorization': idToken // Secure Cognito Authorizer
                        },
                        body: JSON.stringify({
                            email: email || "unknown",
                            activePolicies: 0,
                    ...
                    if (!response.ok) throw new Error("AWS Server Error: " + response.statusText);
                    
                    const data = await response.json();
                    statusText.innerHTML = `<span style="color: #4CAF50;">✓ Successfully saved to DynamoDB via AWS Lambda!</span>`;
                } catch (error) {
                    console.error(error);
                    statusText.innerHTML = `<span style="color: #f44336;">❌ Error syncing with AWS: ${error.message}</span>`;
                } finally {
